...
namespace Microsoft.MixedReality.Toolkit.Examples.Demos.EyeTracking
{
    ...
    public class TargetGroupCreatorRadial : MonoBehaviour
    {
        ...
        //[SerializeField]
        private Vector3 targetSizeInVisAngle = new Vector3(0.2f, 0.2f, 0.2f);
        ...
        //[SerializeField]
        private int radialLayout_nTargets = 9;
        ...
        private void Start()
        {
            ...
            //var flourD = ConeRangeSelect.GetComponent<flourDeployment> ();
            targetposi = flourDeployment.TargetPosition; 
        ...
        private void CreateNewTargets_RadialLayout()
        {
            ...
            Vector3 square = new Vector3(3, 3, 3);

            // Let's make sure we have some templates to work with
            if ((templates != null) && (templates.Length > 0))
            {

                for (int ix = 0; ix < square.x; ix++)
                {
                
                    for (int jy = 0; jy < square.y; jy++)
                    {
                        for (int kz = 0; kz < square.z; kz++)
                        {
                             InstantiateRadialLayoutedTarget(ix, jy, kz);
                        }
        ...
        private void InstantiateRadialLayoutedTarget(float i, float j, int k)
        {
             GameObject _target;
            
            if(i == flourDeployment.TargetPosition.x && j == flourDeployment.TargetPosition.y && k == flourDeployment.TargetPosition.z){
                _target = Instantiate(templates[1]);
            }
            else{
                _target = Instantiate(templates[0]);
            }

            //GameObject _target = Instantiate(templates[0]);

            // Length between elements
            Vector3 len = new Vector3(0.4f, 0.4f, 0.4f);

            // Position
            Vector3 initialpoint = new Vector3(-0.5f,-0.5f,2.0f);
            float xnew = i*len.x + initialpoint.x; 
            float ynew = j*len.y + initialpoint.y;
            float znew = k*len.z + initialpoint.z;

            //_target.transform.localPosition = new Vector3(xnew, ynew, znew);
            _target.transform.position = new Vector3(xnew, ynew, znew);

            // Scale
            Vector3 kero = new Vector3(0.15f, 0.15f, 0.15f);
            Vector3 kero2 = new Vector3(0.1f, 0.1f, 0.1f);

            _target.transform.localScale = kero;
            if(i == flourDeployment.TargetPosition.x && j == flourDeployment.TargetPosition.y && k == flourDeployment.TargetPosition.z){
                _target.transform.localScale = kero2;
            }

            // Name it
            _target.name = string.Format("target_x{0}_y{1}_z{2}", i, j, k);
